    ...
    try:
        ...
        if quantidade <= 0:
            ...
            for i in range(quantidade):
                nome_input = input(f"Nome do {i + 1}o livro: ")
                if nome_input == "":
                    break
                ...
                print(f"Livro {nome_input} adicionado a sua lista com sucesso!\n")
...
def alteracao(livro, alter):
    ...
    if mudar == "Nome":
        ...
        status = input(f"Digite 'S' para concluir: ").upper()
        if status == "S":
...
def estatisticas(lista_cadastros):
    print("\n--- STATUS PORTFÓLIO ---")
    total_projetos = len(lista_cadastros)
    if total_projetos == 0:
    ...
    print(f"Taxa de conclusão: {concluidos * 100/total_projetos:.2f}%")
